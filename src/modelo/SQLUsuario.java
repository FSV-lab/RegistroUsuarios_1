...
public class SQLUsuario {
  ...
  public int verificarUsuario(String usuario){
      ...
      try {
          ...
          ps = conexion.prepareStatement("select count(id) from usuario where nombreUsuario=?");//conteo de cuantos nombres existen
          ps.setString(1, usuario);
          
          rs = ps.executeQuery();
          
          if (rs.next()) {
            return rs.getInt(1);//valor del conteo que estamos haciendo
          }
          return 1;
      } catch (Exception ex) {
          return 1;
